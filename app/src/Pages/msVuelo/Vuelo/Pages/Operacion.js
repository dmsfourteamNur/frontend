      ...
      <SPage title={"Operacion"}>
        <STable2
          ...
          header={[
            ...
            //   render: (keyAeronave) => {
			// 	// var obj = this.state.dataVuelo.find((o) => o.keyAeronave == "686fc732-d731-4b29-beae-1ff15816eedb");
			// 	// this.test(keyAeronave);
			// }
 			},
            { key: "origen", label: "origen", width: 130, center: true },
            { key: "destino", label: "destino", width: 130, center: true },
            // { key: "fechaSalida",label: "test fecha",width: 130,center: true,render: (item) => { return new SDate(item).toString("yyyy-MM-dd hh:mm:ss")}},
            { key: "fechaSalida",label: "Fecha Salida",width: 130,center: true,render: (item) => { return new SDate(item).toString("yyyy-MM-dd")}},
            // { key: "fechaSalida",label: "Hora Salida",width: 130,center: true, render: (item) => { return new SDate(item).toString("hh:mm:ss")}},
            { key: "fechaArribe",label: "Fecha Lllegada",width: 130,center: true,render: (item) => { return new SDate(item).toString("yyyy-MM-dd")}},
            // { key: "fechaArribe",label: "Hora Lllegada",width: 130,center: true, render: (item) => { return new SDate(item).toString("hh:mm:ss")}},
              ...
              component: (key) => {
                return (
                  <SView width={35} height={35}
                    onPress={() => { var obj = this.state.dataVuelo.find((o) => o.key == key);
                      SPopup.confirm({ title: "Eliminar", message: "¿Esta seguro de eliminar?",
